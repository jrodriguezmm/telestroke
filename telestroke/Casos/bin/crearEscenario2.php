...
if ( (!isset($_POST['IdCaso'])) || (!isset($_POST['UsuarioID'])) || (!isset($_POST['IdIPSEscenario'])) || (!isset($_POST['ConsecutivoAdmision']))  ){
	...
	$sqlInsert = "INSERT INTO escenario2(
		IdCaso,
		IdIPSEscenario,
		EstadoEdicion,
		EscenarioCerrado,
	...
	$resultado = mysqli_query($con, $sqlInsert) or die("Error al insertar Escenario 2: " . mysqli_error($con) );
			
		if ($resultado <> 1){
			echo "Fallo insertar Caso:" . $sqlInsert;
		} else
		{
			$sql = "SELECT * FROM escenario2 WHERE UsuarioID='" . $_POST['UsuarioID'] . "' AND IdCaso = '" . $_POST['IdCaso'] . "' ORDER BY IdEscenario DESC LIMIT 1";
			...
			if ($resultado) {
				...
				if ($numfilas == 1) {
					...
					echo "Falló insertar Escenario 2. numfilas=" . $numfilas . " sql: " . $sql ;
				}
				
			} else {
				echo "Falló insertar Escenario 2. No se verificó el insert:" . $sql;
			}
		}
}
